...
def _haxe_gen_hxml(ctx):
    ...
    for classpath in hxml["classpaths"]:
        if classpath.startswith("external"):
            temp_classpaths.append("{}/{}".format(ctx.attr.bazel_workspace_path, classpath))
        else:
            temp_classpaths.append(classpath)
    hxml["classpaths"] = temp_classpaths

    build_file_name = ctx.attr.hxml_name if hasattr(ctx.attr, "hxml_name") else ctx.attr.name
...
haxe_gen_hxml = rule(
    ...
    attrs = {
        ...
        "bazel_workspace_path": attr.string(
            doc = "The name of the 'bazel-my-project' link generated by bazel; this is required to resolve dependency paths.",
            mandatory = True,
        ),
        "srcs": attr.label_list(
            mandatory = True,
