...
load("//:repo.bzl", _haxe_download = "haxe_download", _haxe_download_linux_amd64 = "haxe_download_linux_amd64", _haxe_download_universal = "haxe_download_universal", _haxe_download_windows_amd64 = "haxe_download_windows_amd64", _haxe_no_install = "haxe_no_install")
...
def _haxe_install(ctx):
    for mod in ctx.modules:
        ...
        for install_universal in mod.tags.install_universal:
            haxe_download_universal(name = install_universal.name)

_no_install = tag_class(attrs = {"name": attr.string()})
_install_universal = tag_class(attrs = {"name": attr.string()})
haxe_install_ext = module_extension(
    implementation = _haxe_install,
    tag_classes = {"no_install": _no_install, "install_universal": _install_universal},
)
